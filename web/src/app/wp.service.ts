...
export class WpService {
    ...
    async manipulateContentBrut(contentEmpty : any, allDatas : any)
    {
		...
		const regex2 = /<!--\s*wp:uagb\/icon-list\s*{.*?}\s*-->/g;
		const regex3 = /<!--\s*wp:uagb\/icon-list-child\s*{.*?}\s*-->/g;
		const regex4 = /<!--\s*\/wp:uagb\/icon-list-child\s*-->/g;


        // Supprimer le bloc <div> et son contenu de la chaîne HTML
        contentFilled = contentFilled.replace(regex, '');    
		// Supprimer tous les blocs correspondants de la chaîne HTML
		contentFilled = contentFilled.replace(regex2, '');
		contentFilled = contentFilled.replace(regex3, '');
		contentFilled = contentFilled.replace(regex4, '');
        //#############################


        // Expression régulière pour capturer le bloc spécifique où vous souhaitez ajouter des enfants
        const regexFinal = /(<div class="wp-block-uagb-icon-list uagb-block-(.*?) listLinkSourceVideos">)\s*<div class="uagb-icon-list__wrap">/;

        // Ajouter le nouveau contenu à l'intérieur du bloc spécifié
        contentFilled = contentFilled.replace(regexFinal, `$1<div class="uagb-icon-list__wrap">${allDatas.allDatasWithLink}`);

        console.log(contentFilled)
    ...
    async manipulationLinkVideos(allLinks : any){
        ...
        allLinks.forEach((each : any) => {
            ...
            <span class="uagb-icon-list__label">${each.text}</span></div>`
        })
